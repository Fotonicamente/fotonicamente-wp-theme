...
//Enqueue new fotonicamente Style
function enqueue_fotonicamente_css_style(){
    wp_register_style(
        'fotonicamente-custom-style',
        get_bloginfo('template_directory') . '/css/fotonicamente.css',
        false,
        0.1
    );
    wp_enqueue_style('fotonicamente-custom-style');
}

if (!is_admin()) {
    add_action( 'wp_enqueue_scripts', 'enqueue_fotonicamente_css_style', 99); // using priority 99 we make s
}


// LOGO PERSONALIZZATO
add_action('login_head', 'custom_loginlogo_FM');
function custom_loginlogo_FM()
{
    echo '<style type="text/css"> h1 a {background-image: url(' . get_bloginfo('template_directory') . '/images/FM-login.png) !important; } </style>';
}
 ...
 * @return string
 */
function my_fm_login_redirect($redirect_to, $request, $user)
{
    ...
    if (isset($user->roles) && is_array($user->roles)) {
        ...
        if (in_array('administrator', $user->roles)) {
            ...
            return "http://www.fotonicamente.it/wp/wp-admin/";
        } else {
            return "http://www.fotonicamente.it";
        }
    } else {
        return $redirect_to;
    }
...
add_filter('login_redirect', 'my_fm_login_redirect', 10, 3);
